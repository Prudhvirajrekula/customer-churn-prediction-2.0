...
# --- Page Configuration ---
st.set_page_config(page_title="Churn & LTV Intelligence Platform", layout="wide")
...
# --- Tabs ---
tab1, tab2, tab3 = st.tabs(["🔍 Predict Customer Risk", "📊 Segment Explorer", "💬 GenAI Bot Insights"])

# --- TAB 1: Prediction ---
with tab1:
    st.title("🔍 Predict Churn & Lifetime Value")
    st.markdown("Use the slider to select a customer and compute real-time predictions for churn risk and expected lifetime value.")
    ...
    col1.metric("Churn Probability", f"{churn_proba:.0%}", "⚠️ High" if churn_proba > 0.5 else "✅ Low")
    col2.metric("Predicted LTV", f"${ltv_pred:,.0f}")

    st.markdown(f"### 🔎 Risk Assessment: {'⚠️ **High Risk**' if churn_proba > 0.5 else '✅ **Low Risk**'}")
...
with tab2:
    ...
    st.markdown("Analyze behavioral clusters of customers based on usage, recency, and payment trends.")

    if segments_df is None:
        st.warning("No segmentation data available. Please generate clusters.")
    else:
        selected_cluster = st.selectbox("Select Cluster", sorted(segments_df["cluster"].unique()))
        ...
        if "persona" in filtered_df.columns:
            st.markdown(f"### 🧠 Dominant Persona: `{filtered_df['persona'].iloc[0]}`")
...
""")
    ...
    st.markdown("Click any of the common questions below to get instant insights from Muffin:")
